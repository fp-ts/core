 ...
 *
 * @param e - The error value to put in the `Left` of the resulting `Validated`.
 *
 * @example
 * import * as E from '@fp-ts/core/Either'
 *
 ...
 *
 * @param self - The `Either` value to convert.
 *
 * @example
 * import * as E from '@fp-ts/core/Either'
 *
 ...
 * @since 1.0.0
 */
export const toValidated: <E, A>(self: Either<E, A>) => Validated<E, A> = mapLeft((e) => [e])

/**
 * Lifts an `Either`-returning function into a function that returns a `Validated`.
 *
 * @param f - The `Either`-returning function to lift.
 *
 * @example
 * import * as E from '@fp-ts/core/Either'
 *
 ...
 * @since 1.0.0
 */
export const SemiProductValidated: semiProduct.SemiProduct<ValidatedTypeLambda> = {
  imap,
  product: productValidated,
  productMany: productManyValidated
}

const productAllValidated = <E, A>(
  collection: Iterable<Validated<E, A>>
): Validated<E, Array<A>> => {
  const as = readonlyArray.fromIterable(collection)
  return productManyValidated(as[0], as.slice(1))
}
 ...
 * @since 1.0.0
 */
export const ProductValidated: product_.Product<ValidatedTypeLambda> = {
  imap,
  of,
  product: productValidated,
  productMany: productManyValidated,
  productAll: productAllValidated
}
 ...
 *
 * @param elements - A tuple of `Validated` values.
 *
 * @example
 * import * as E from '@fp-ts/core/Either'
 *
 ...
 * @since 1.0.0
 */
export const tupleValidated: <T extends ReadonlyArray<Validated<any, any>>>(
  ...elements: T
) => Validated<
  [T[number]] extends [Validated<infer E, any>] ? E : never,
  { [I in keyof T]: [T[I]] extends [Validated<any, infer A>] ? A : never }
> = product_.tuple(ProductValidated)
 ...
 * Errors are accumulated.
 *
 * @param fields - A struct of `Validated` values.
 *
 * @example
 * import * as E from '@fp-ts/core/Either'
 *
 ...
 * @since 1.0.0
 */
export const structValidated: <R extends { readonly [x: string]: Validated<any, any> }>(
  fields: R
) => Validated<
  [R[keyof R]] extends [Validated<infer E, any>] ? E : never,
  { [K in keyof R]: [R[K]] extends [Validated<any, infer A>] ? A : never }
> = product_.struct(ProductValidated)

/**
 * @category instances
 * @since 1.0.0
 */
export const SemiApplicativeValidated: semiApplicative.SemiApplicative<ValidatedTypeLambda> = {
  imap,
  map,
  product: productValidated,
  productMany: productManyValidated
 ...
 * Errors are accumulated.
 *
 * @param self - first `Validated` value to compose.
 * @param that - second `Validated` value to compose.
 * @param f - binary function used to compute the `Validated` result from `self` and `that`.
 *
 * @example
 * import * as E from '@fp-ts/core/Either'
 * import * as N from '@fp-ts/core/Number'
...
} = semiApplicative.zipWith(SemiApplicativeValidated)
 ...
 * @since 1.0.0
 */
export const lift2Validated: <A, B, C>(
  f: (a: A, b: B) => C
) => {
...
} = semiApplicative.lift2(SemiApplicativeValidated)

/**
 * @category instances
 * @since 1.0.0
 */
export const ApplicativeValidated: applicative.Applicative<ValidatedTypeLambda> = {
  imap,
  map,
  of,
  product: productValidated,
