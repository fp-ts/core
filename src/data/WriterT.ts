...
export const map = <F extends TypeLambda>(Functor: Functor<F>) => {
  ...
  return <A, B>(
    ...
    self: Kind<WriterT<F, W>, S, R, O, E, A>
  ) => Kind<WriterT<F, W>, S, R, O, E, B>) => map_(f)
}
...
) => Kind<WriterT<F, W>, S, R1 & R2, O1 | O2, E1 | E2, B>) =>
  apply.apComposition(Apply, writer.getApply(Semigroup))

/**
 * @since 3.0.0
 */
export const flatMap = <F extends TypeLambda, W>(
...
export const mapBoth = <F extends TypeLambda>(
  ...
  self: Kind<WriterT<F, W>, S, R, O, E, A>
) => Kind<WriterT<F, X>, S, R, O, E, B>) => bifunctor.mapBothComposition(Functor, writer.Bifunctor)
...
export const reverse = <F extends TypeLambda>(
  ...
  self: Kind<WriterT<F, W>, S, R, O, E, A>
) => Kind<WriterT<F, A>, S, R, O, E, W>) => Functor.map(writer.reverse)
...
export const listen = <F extends TypeLambda>(
  ...
  self: Kind<WriterT<F, W>, S, R, O, E, A>
) => Kind<WriterT<F, W>, S, R, O, E, readonly [W, A]>) => Functor.map(writer.listen)
...
export const listens = <F extends TypeLambda>(Functor: Functor<F>) =>
  <W, B>(
    ...
    self: Kind<WriterT<F, W>, S, R, O, E, A>
  ) => Kind<WriterT<F, W>, S, R, O, E, readonly [A, B]>) => Functor.map(writer.listens(f))
...
export const censor = <F extends TypeLambda>(Functor: Functor<F>) =>
  <W>(
    ...
    self: Kind<WriterT<F, W>, S, R, O, E, A>
  ) => Kind<WriterT<F, W>, S, R, O, E, A>) => Functor.map(writer.censor(f))
