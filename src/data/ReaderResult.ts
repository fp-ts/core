 ...
 * @since 3.0.0
 */
export const succeed: <A>(a: A) => ReaderResult<unknown, never, A> = resultT.succeed(
  reader.FromIdentity
)
 ...
 * @since 3.0.0
 */
export const asksReaderResult: <R1, R2, E, A>(
  f: (r1: R1) => ReaderResult<R2, E, A>
) => ReaderResult<R1 & R2, E, A> = reader.asksReader
...
export const match: <E, B, A, C = B>(
  onError: (e: E) => B,
  onSuccess: (a: A) => C
) => <R>(ma: Reader<R, Result<E, A>>) => Reader<R, B | C> = resultT.match(reader.Functor)
...
) => <R1>(ma: Reader<R1, Result<E, A>>) => Reader<R1 & R2 & R3, B | C> = resultT.matchKind(
  reader.Monad
)
...
export const getOrElse: <B>(
  onError: B
) => <R, A>(self: ReaderResult<R, unknown, A>) => Reader<R, A | B> = resultT.getOrElse(
  reader.Functor
)
...
export const local: <R2, R1>(
  f: (r2: R2) => R1
) => <E, A>(ma: ReaderResult<R1, E, A>) => ReaderResult<R2, E, A> = reader.local
 ...
 * @since 3.0.0
 */
export const reverse: <R, E, A>(ma: ReaderResult<R, E, A>) => ReaderResult<R, A, E> = resultT
  .reverse(
    reader.Functor
  )
...
export const map: <A, B>(
  f: (a: A) => B
) => <R, E>(fa: ReaderResult<R, E, A>) => ReaderResult<R, E, B> = resultT.map(reader.Functor)
...
export const mapError: <E, G>(
  f: (e: E) => G
) => <R, A>(self: ReaderResult<R, E, A>) => ReaderResult<R, G, A> = resultT.mapError(reader.Functor)
...
export const flatMapError: <E1, R, E2>(
  f: (e: E1) => Reader<R, E2>
) => <A>(self: ReaderResult<R, E1, A>) => ReaderResult<R, E2, A> = resultT.flatMapError(
  reader.Monad
)
...
export const composeKleisli: <B, R2, E2, C>(
  ...
  afb: (a: A) => ReaderResult<R1, E1, B>
) => (a: A) => ReaderResult<R1 & R2, E2 | E1, C> = flattenable.composeKleisli(Flattenable)
...
) => <R1, E1, A>(self: ReaderResult<R1, E1, A>) => ReaderResult<R1 & R2, E2, A | B> = resultT
  .orElse(
    reader.Monad
  )
...
export const compact: <E>(
  onNone: E
) => <R, A>(self: ReaderResult<R, E, Option<A>>) => ReaderResult<R, E, A> = resultT.compact(
  reader.Functor
)
...
export const separate: <E>(
  ...
  self: ReaderResult<R, E, Result<A, B>>
) => readonly [ReaderResult<R, E, A>, ReaderResult<R, E, B>] = resultT.separate(reader.Functor)
...
export const getCompactable = <E>(
  onNone: E
): Compactable<result.ValidatedT<ReaderResultTypeLambda, E>> => {
  return {
...
export const flap: <A>(
  a: A
) => <R, E, B>(fab: ReaderResult<R, E, (a: A) => B>) => ReaderResult<R, E, B> = functor.flap(
  Functor
)
 ...
 * @since 3.0.0
 */
export const unit: <R, E>(self: ReaderResult<R, E, unknown>) => ReaderResult<R, E, void> = functor
  .unit(Functor)
...
) => ReaderResult<R1 & R2, E1 | E2, C> = apply.lift2(Apply)
...
) => (collection: Iterable<ReaderResult<R, E, A>>) => ReaderResult<R, E, A> = alt.firstSuccessOf(
  Alt
)
 ...
 * @since 3.0.0
 */
export const asks: <R, A>(f: (r: R) => A) => ReaderResult<R, never, A> = fromReader_.asks(
  FromReader
)
...
export const flatMapOption: <A, B, E2>(
  f: (a: A) => Option<B>,
  onNone: E2
) => <R, E1>(self: ReaderResult<R, E1, A>) => ReaderResult<R, E2 | E1, B> = fromResult_
  .flatMapOption(
    FromResult,
    Flattenable
  )
...
export const liftPredicate: {
  <C extends A, B extends A, E, A = C>(
    ...
    onFalse: E
  ): (c: C) => ReaderResult<unknown, E, B>
  <B extends A, E, A = B>(
    predicate: Predicate<A>,
    onFalse: E
  ): (b: B) => ReaderResult<unknown, E, B>
...
export const fromNullable: <E>(
  onNullable: E
) => <A>(a: A) => ReaderResult<unknown, E, NonNullable<A>> = fromResult_.fromNullable(FromResult)
...
) => ReaderResult<R, E, { readonly [K in N | keyof A]: K extends keyof A ? A[K] : B }> = functor
  .let(Functor)
...
export const zipWith: <R2, E2, B, A, C>(
  that: ReaderResult<R2, E2, B>,
  f: (a: A, b: B) => C
) => <R1, E1>(self: ReaderResult<R1, E1, A>) => ReaderResult<R1 & R2, E2 | E1, C> = apply.zipWith(
  Apply
)
...
export const traverseNonEmptyReadonlyArrayWithIndex = <A, R, E, B>(
  ...
  flow(
    ...
    reader.map(result.traverseNonEmptyReadonlyArrayWithIndex(SK))
  )
