...
export const failKind = <F extends TypeLambda>(
  Functor: Functor<F>
): (<S, R, O, FE, E>(fl: Kind<F, S, R, O, FE, E>) => Kind<TheseT<F, E>, S, R, O, FE, never>) =>
  Functor.map(these.fail)
...
export const map = <F extends TypeLambda>(Functor: Functor<F>) =>
  <A, B>(
    ...
    self: Kind<TheseT<F, E>, S, R, O, FE, A>
  ) => Kind<TheseT<F, E>, S, R, O, FE, B>) => functor.mapComposition(Functor, these.Functor)(f)
...
export const ap = <F extends TypeLambda, E>(
  ...
  apply.apComposition(Apply, these.getApply(Semigroup))
...
export const mapBoth = <F extends TypeLambda>(
  ...
  self: Kind<TheseT<F, E>, S, R, O, FE, A>
) => Kind<TheseT<F, G>, S, R, O, FE, B>) => bifunctor.mapBothComposition(Functor, these.Bifunctor)
...
export const mapError = <F extends TypeLambda>(Functor: Functor<F>) =>
  <E, G>(
    ...
    self: Kind<TheseT<F, E>, S, R, O, FE, A>
  ) => Kind<TheseT<F, G>, S, R, O, FE, A>) => Functor.map(these.mapError(f))
...
export const matchKind = <F extends TypeLambda>(Flattenable: Flattenable<F>) =>
  <E, S, R2, O2, FE2, B, A, R3, O3, FE3, R4, O4, FE4, C = B, D = B>(
    ...
    Flattenable.flatMap(
      these.match<E, Kind<F, S, R2 & R3 & R4, O2 | O3 | O4, FE2 | FE3 | FE4, B | C | D>, A>(
        onError,
        onSuccess,
        onBoth
      )
...
export const reverse = <F extends TypeLambda>(
  ...
  self: Kind<TheseT<F, E>, S, R, O, FE, A>
) => Kind<TheseT<F, A>, S, R, O, FE, E>) => Functor.map(these.reverse)
...
export const toTuple2 = <F extends TypeLambda>(
  ...
  self: Kind<TheseT<F, E>, S, R, O, FE, A>
) => Kind<F, S, R, O, FE, readonly [E, A]>) => flow(these.toTuple2, Functor.map)
