...
export interface ReaderT<F extends TypeLambda, R> extends TypeLambda {
  readonly type: Reader<
    R,
    Kind<F, this["InOut1"], this["In1"], this["Out3"], this["Out2"], this["Out1"]>
  >
}
...
export const map = <F extends TypeLambda>(Functor: Functor<F>) =>
  <A, B>(f: (a: A) => B) =>
    <R, S, FR, O, E>(
      fa: Kind<ReaderT<F, R>, S, FR, O, E, A>
    ): Kind<ReaderT<F, R>, S, FR, O, E, B> => flow(fa, Functor.map(f))
