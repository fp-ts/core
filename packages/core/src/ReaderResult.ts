...
) => <R1>(ma: Reader<R1, Result<E, A>>) => Reader<R1 & R2 & R3, B | C> =resultT.matchKind(reader.Monad)
...
export const getOrElse: <B>(onError: B) => <R, A>(self: ReaderResult<R, unknown, A>) => Reader<R, A | B> =
 resultT.getOrElse(reader.Functor)
 ...
 * @since 3.0.0
 */
export const reverse: <R, E, A>(ma: ReaderResult<R, E, A>) => ReaderResult<R, A, E> =resultT.reverse(
  reader.Functor
)
...
export const mapError: <E, G>(f: (e: E) => G) => <R, A>(self: ReaderResult<R, E, A>) => ReaderResult<R, G, A> =
 resultT.mapError(reader.Functor)
...
export const compact: <E>(onNone: E) => <R, A>(self: ReaderResult<R, E, Option<A>>) => ReaderResult<R, E, A> =
 resultT.compact(reader.Functor)
...
) => (collection: Iterable<ReaderResult<R, E, A>>) => ReaderResult<R, E, A> =alt.firstSuccessOf(Alt)
