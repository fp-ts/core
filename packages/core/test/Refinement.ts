...
describe('Refinement', () => {
  ...
  it('and', () => {
    const ra = (r: Readonly<Record<string, unknown>>): r is { readonly a: string } => S.isString(r['a'])
    const rb = (r: Readonly<Record<string, unknown>>): r is { readonly b: number } => N.isNumber(r['b'])
