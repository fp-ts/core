...
import * as _ from "@fp-ts/core/data/Result"
...
describe("Result", () => {
  ...
  it("separate", () => {
    ...
    U.deepStrictEqual(pipe(_.succeed(_.fail("e1")), _.separate("e2")), [
      _.succeed("e1"),
      _.fail("e2")
    ])
  ...
  it("partitionMap", () => {
    ...
    U.deepStrictEqual(pipe(_.fail("123"), _.partitionMap(f, S.Monoid.empty)), [
      _.fail("123"),
      _.fail("123")
    ])
    U.deepStrictEqual(pipe(_.succeed(1), _.partitionMap(f, S.Monoid.empty)), [
      _.succeed(0),
      _.fail(S.Monoid.empty)
    ])
    U.deepStrictEqual(pipe(_.succeed(3), _.partitionMap(f, S.Monoid.empty)), [
      _.fail(S.Monoid.empty),
      _.succeed(4)
    ])
  })
  ...
  describe("getFilterableKind", () => {
    ...
    it("partitionMapKind", async () => {
      ...
      U.deepStrictEqual(await pipe(_.fail("foo"), partitionMapKind(f))(), [
        _.fail("foo"),
        _.fail("foo")
      ])
      U.deepStrictEqual(await pipe(_.succeed(1), partitionMapKind(f))(), [
        _.succeed(0),
        _.fail(S.Monoid.empty)
      ])
      U.deepStrictEqual(await pipe(_.succeed(3), partitionMapKind(f))(), [
        _.fail(S.Monoid.empty),
        _.succeed(4)
      ])
    })
  ...
  it("getApplicativeValidation", () => {
    ...
    const flatZipPar = <B>(fb: _.Result<string, B>) =>
      <A extends ReadonlyArray<unknown>>(
        fas: _.Result<string, A>
      ): _.Result<string, readonly [...A, B]> =>
        pipe(
          fas,
  ...
  it("sequenceReadonlyArray", () => {
    U.deepStrictEqual(
      pipe([_.succeed("a"), _.succeed("b")], _.sequenceReadonlyArray),
      _.succeed(["a", "b"])
    )
