...
import * as _ from "@fp-ts/core/data/SyncResult"
...
describe("SyncResult", () => {
  ...
  it("tap", () => {
    const f = (
      a: string
    ): _.SyncResult<string, number> => (a.length > 2 ? _.succeed(a.length) : _.fail("foo"))
  ...
  it("partitionMap", () => {
    ...
    assertPartition(pipe(_.fail("123"), _.partitionMap(f, S.Monoid.empty)), [
      E.fail("123"),
      E.fail("123")
    ])
    assertPartition(pipe(_.succeed(1), _.partitionMap(f, S.Monoid.empty)), [
      E.succeed(0),
      E.fail(S.Monoid.empty)
    ])
    assertPartition(pipe(_.succeed(3), _.partitionMap(f, S.Monoid.empty)), [
      E.fail(S.Monoid.empty),
      E.succeed(4)
    ])
  })
  ...
  it("zipFlatten", () => {
    U.deepStrictEqual(
      pipe(_.succeed(1), _.tupled, _.zipFlatten(_.succeed("b")))(),
  ...
  it("traverseReadonlyArrayWithIndexPar", () => {
    U.deepStrictEqual(
      pipe(RA.empty, _.traverseReadonlyArrayWithIndexPar(SK))(),
      E.succeed(RA.empty)
    )
    ...
    U.deepStrictEqual(
      pipe([right(1), right(2)], _.traverseReadonlyArrayWithIndexPar(SK))(),
      E.succeed([1, 2])
    )
    U.deepStrictEqual(
      pipe([right(3), left("a")], _.traverseReadonlyArrayWithIndexPar(SK))(),
      E.fail("a")
    )
    U.deepStrictEqual(
      pipe([left("b"), right(4)], _.traverseReadonlyArrayWithIndexPar(SK))(),
      E.fail("b")
    )
  ...
  it("sequenceReadonlyArray", () => {
    ...
    U.deepStrictEqual(
      pipe([right(1), right(2)], _.traverseReadonlyArrayWithIndex(SK))(),
      E.succeed([1, 2])
    )
    U.deepStrictEqual(
      pipe([right(3), left("a")], _.traverseReadonlyArrayWithIndex(SK))(),
      E.fail("a")
    )
    U.deepStrictEqual(
      pipe([left("b"), right(4)], _.traverseReadonlyArrayWithIndex(SK))(),
      E.fail("b")
    )
