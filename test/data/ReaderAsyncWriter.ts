...
describe("ReaderAsyncWriter", () => {
  ...
  it("fromAsyncWriter", async () => {
    U.deepStrictEqual(await pipe(T.of(W.tell("a")), _.fromAsyncWriter)(undefined)(), [
      "a",
      undefined
    ])
  })
  ...
  it("getFromIdentity", async () => {
    const P = _.getFromIdentity(S.Monoid)
    U.deepStrictEqual(await P.of(1)(undefined)(), ["", 1])
  })

  it("getApply", async () => {
    const A = _.getApply(RT.ApplyPar, S.Semigroup)
    const f = (s: string) => (n: number): boolean => s.length > n
    U.deepStrictEqual(await pipe(make("a", "aa"), A.map(f), A.ap(make("b", 1)))(undefined)(), [
      "ab",
      true
    ])
  })
  ...
  it("getApplicative", async () => {
    const A = _.getApplicative(RT.ApplyPar, S.Monoid)
    const f = (s: string) => (n: number): boolean => s.length > n
    U.deepStrictEqual(await pipe(A.of("aa"), A.map(f), A.ap(A.of(1)))(undefined)(), ["", true])
  })

  it("getFlattenable", async () => {
    const C = _.getFlattenable(S.Semigroup)
    const double = (n: number): _.ReaderAsyncWriter<unknown, string, number> =>
      make("double", n * 2)
    U.deepStrictEqual(await pipe(make("start", 1), C.flatMap(double))(undefined)(), [
      "startdouble",
      2
    ])
  })
  ...
  it("sequenceReadonlyArray", async () => {
    ...
    U.deepStrictEqual(await pipe([of("a"), of("b")], sequenceReadonlyArray)(null)(), ["", [
      "a",
      "b"
    ]])
  })
