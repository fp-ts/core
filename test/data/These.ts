...
describe("These", () => {
  ...
  it("getMonad", () => {
    ...
    const f = (
      n: number
    ) => (n >= 2 ? (n <= 5 ? _.succeed(n * 2) : _.both("bar", n)) : _.fail("bar"))
  ...
  it("traverseReadonlyArrayWithIndex", () => {
    const f = (
      i: number,
      n: number
    ) => (n > 0 ? _.succeed(n + i) : n === 0 ? _.both("a", 0) : _.fail(String(n)))
  ...
  it("sequenceReadonlyArray", () => {
    ...
    U.deepStrictEqual(
      pipe([_.succeed("a"), _.succeed("b")], sequenceReadonlyArray),
      _.succeed(["a", "b"])
    )
