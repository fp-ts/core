...
import * as RTE from "@fp-ts/core/data/ReaderAsyncResult"
import * as RE from "@fp-ts/core/data/ReaderResult"
import * as RA from "@fp-ts/core/data/ReadonlyArray"
import * as E from "@fp-ts/core/data/Result"
import type { State } from "@fp-ts/core/data/State"
import * as _ from "@fp-ts/core/data/StateReaderAsyncResult"
...
describe("StateReaderAsyncResult", () => {
  describe("pipeables", () => {
    ...
    it("filter", async () => {
      ...
      U.deepStrictEqual(
        await pipe(_.succeed(12), _.filter(predicate, -1), _.evaluate(state))({})(),
        E.succeed(12)
      )
      U.deepStrictEqual(
        await pipe(_.succeed(7), _.filter(predicate, -1), _.evaluate(state))({})(),
        E.fail(-1)
      )
      U.deepStrictEqual(
        await pipe(_.fail(12), _.filter(predicate, -1), _.evaluate(state))({})(),
        E.fail(12)
      )
    })
  })
  ...
  it("applicativeReaderAsyncResultSeq", async () => {
    ...
    const sequence = RA.traverse(_.Applicative)(<S, R, E, A>(
      a: _.StateReaderAsyncResult<S, R, E, A>
    ) => a)
  ...
  it("tapError", async () => {
    ...
    U.deepStrictEqual(
      await pipe(_.succeed<number, number>(1), f)(0)(null)(),
      E.succeed([0, 1] as const)
    )
  ...
  it("traverseReadonlyArrayWithIndex", async () => {
    ...
    U.deepStrictEqual(
      await pipe(RA.empty, f)(undefined)(undefined)(),
      E.succeed([undefined, RA.empty] as const)
    )
    U.deepStrictEqual(
      await pipe(["a", "b"], f)(undefined)(undefined)(),
      E.succeed([undefined, ["a0", "b1"]] as const)
    )
    U.deepStrictEqual(await pipe(["a", ""], f)(undefined)(undefined)(), E.fail("e"))
    const append = (
      _i: number,
      n: number
    ): _.StateReaderAsyncResult<ReadonlyArray<number>, {}, Error, void> =>
      _.modify((a) => [...a, n])
    U.deepStrictEqual(
      await pipe(
        [1, 2, 3],
  ...
  it("traverseNonEmptyReadonlyArray", async () => {
    ...
    U.deepStrictEqual(
      await pipe(["a", "b"], f)(undefined)(undefined)(),
      E.succeed([undefined, ["a", "b"]] as const)
    )
    U.deepStrictEqual(await pipe(["a", ""], f)(undefined)(undefined)(), E.fail("e"))
  ...
  it("sequenceReadonlyArray", async () => {
    ...
    U.deepStrictEqual(
      await pipe([right(3), left("a")], _.sequenceReadonlyArray)(undefined)(undefined)(),
      E.fail("a")
    )
    U.deepStrictEqual(
      await pipe([left("b"), right(4)], _.sequenceReadonlyArray)(undefined)(undefined)(),
      E.fail("b")
    )
