...
import * as TO from "@fp-ts/core/data/AsyncOption"
import * as _ from "@fp-ts/core/data/AsyncResult"
...
describe("AsyncResult", () => {
  ...
  it("Applicative", async () => {
    await assertSeq(
      (a, b) => pipe(a, _.Apply.map(S.Semigroup.combine), _.Apply.ap(b)),
      E.succeed("ba")
    )
    await assertSeq(
      (a, b) => pipe(a, _.Applicative.map(S.Semigroup.combine), _.Applicative.ap(b)),
      E.succeed("ba")
    )
  })
  ...
  describe("bracket", () => {
    ...
    it("should return the release error if both use and release fail", async () => {
      U.deepStrictEqual(
        await _.bracket(acquireSuccess, useFailure, releaseFailure)(),
    ...
    it("should return the release error if release fails", async () => {
      U.deepStrictEqual(
        await _.bracket(acquireSuccess, useSuccess, releaseFailure)(),
        E.fail("release failure")
      )
    })
  })
  ...
  it("bindPar", async () => {
    await assertSeq(
      (a, b) => pipe(a, _.bindTo("a"), _.bindRight("b", b)),
      E.succeed({ a: "a", b: "b" })
    )
  })
  ...
  it("sequenceReadonlyArrayPar", async () => {
    ...
    U.deepStrictEqual(
      await pipe([right(1), right(2)], _.sequenceReadonlyArrayPar)(),
      E.succeed([1, 2])
    )
  ...
  it("sequenceReadonlyArray", async () => {
    ...
    U.deepStrictEqual(
      await pipe([right(1), right(2)], _.sequenceReadonlyArray)(),
      E.succeed([1, 2])
    )
